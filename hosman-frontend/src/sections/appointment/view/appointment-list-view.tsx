...
export default function AppointmentListView() {
  ...
  const filteredDepartments = departments
    .filter((dept: any) => dept.count > 0) // Filter out departments with no appointments
    .filter((dept: any) =>
  ...
  const handleDepartmentClick = (id: number) => {
    navigate(
      `${paths.dashboard.Appointment.department.replace(":id", String(id))}`
    );
  };
    ...
    <DashboardContent>
      ...
      <Grid container spacing={3}>
        {filteredDepartments.length > 0 ? (
          ...
          <Typography sx={{ mt: 2 ,ml:3}}>No appointment found.</Typography>
        )}
      </Grid>
    </DashboardContent>
  );
